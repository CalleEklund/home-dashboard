...
const createIntegrationTests = (test, driverFactory) => {
    ...
    test('emits thrown errors', async (t) => {
        ...
        await t.throwsAsync(pool.any(__1.sql.unsafe `SELECT WHERE`));
        t.is(onError.callCount, 1);
        t.true(onError.firstCall.args[0] instanceof __1.InputSyntaxError);
    });
    ...
    `);
      ...
      `));
        t.true(error instanceof __1.CheckExclusionConstraintViolationError);
        await pool.end();
    });
    ...
    `);
        ...
        await t.is(firstConnectionPid, await pool.oneFirst(__1.sql.unsafe `
        SELECT pg_backend_pid();
      `));
        await t.throwsAsync(pool.query(__1.sql.unsafe `
        SELECT pg_terminate_backend(${firstConnectionPid})
      `), {
            instanceOf: __1.BackendTerminatedError,
        });
        const nextConnectionPid = await pool.oneFirst(__1.sql.unsafe `
      SELECT pg_backend_pid();
    `);
    ...
    test('connections failing auth are not added to the connection pool', async (t) => {
        const superPool = await (0, __1.createPool)(t.context.dsn, {
            driverFactory,
            maximumPoolSize: 1,
        });
        const connection = (0, __1.parseDsn)(t.context.dsn);
        const testUser = `auth_change_test_${(0, node_crypto_1.randomUUID)().split('-')[0]}`;
        await superPool.query(__1.sql.unsafe `
        CREATE ROLE ${__1.sql.identifier([testUser])}
        WITH LOGIN SUPERUSER
        PASSWORD 'auth_change_test'
      `);
        // Connect as the new role
        const pool = await (0, __1.createPool)(`postgres://${testUser}:auth_change_test@${connection.host}:${connection.port}/${connection.databaseName}`, {
            driverFactory,
            idleTimeout: 1000,
            maximumPoolSize: 1,
        });
        await pool.oneFirst(__1.sql.unsafe `
      SELECT pg_backend_pid();
    `);
        // Change the password
        await superPool.query(__1.sql.unsafe `
        ALTER ROLE ${__1.sql.identifier([testUser])}
      ...
      `);
        ...
        const error = await t.throwsAsync(pool.oneFirst(__1.sql.unsafe `
        SELECT pg_backend_pid();
      `));
